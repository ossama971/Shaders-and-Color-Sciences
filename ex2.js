/**
 * ex2.js — Exercise 2: Color Elevation Maps
 *
 * Builds a heightfield surface from a still image or video.
 * The user picks a colour space and one of its channels to drive elevation.
 */
...
const SCALE = 0.75; // fixed elevation scale (same as elevation-basic.html)
const DISCRET = 2; // every Nth pixel → vertex
const PLANE_W = 4.0; // world-space width
...
export async function initExercise2() {
  // ---- Scene, camera, renderer (matches elevation-basic.html) ----
  ...
  // ---- Flat reference (original texture below, uses ShaderMaterial like ex1 to bypass color management) ----
  ...
  Object.assign(
    ...
    setupXRExperience({
      ...
      controls,
      title: "Exercise 2",
      description: "Inspect color-driven elevation maps in VR and AR.",
      arWorldYOffset: -0.35,
      xrWorldZOffset: -3.25,
    }),
  );
...
function buildGUI(app) {
  const gui = new GUI({ title: "Elevation Controls" });
...
export function animateExercise2(app) {
